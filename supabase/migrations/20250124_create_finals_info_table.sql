-- 決勝情報テーブルの作成
CREATE TABLE IF NOT EXISTS finals_info (
...
-- 更新時刻を自動更新するトリガー
CREATE OR REPLACE FUNCTION update_finals_info_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER update_finals_info_updated_at
  BEFORE UPDATE ON finals_info
  FOR EACH ROW
  EXECUTE FUNCTION update_finals_info_updated_at();

-- 作成時のユーザーIDを自動設定するトリガー
CREATE OR REPLACE FUNCTION set_finals_info_created_by()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER set_finals_info_created_by
  BEFORE INSERT ON finals_info
  FOR EACH ROW
  EXECUTE FUNCTION set_finals_info_created_by();
...
CREATE POLICY "Users can manage their own finals info" ON finals_info
  ...
  USING (
    auth.uid() IN (
      SELECT user_id FROM entries WHERE id = finals_info.entry_id
    )
  );
