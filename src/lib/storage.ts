...
export async function uploadFile(options: FileUploadOptions): Promise<FileUploadResult> {
  ...
  // 課金後の実際の制限: 動画500MB、その他200MB
  const maxSizeInBytes = fileType === 'video' ? 500 * 1024 * 1024 : 200 * 1024 * 1024
  const maxSizeText = fileType === 'video' ? '500MB' : '200MB'
  ...
  logger.debug('Upload attempt', {
    ...
    metadata: {
      ...
      fileType: file.type,
      fileSize: file.size,
      maxSize: fileType === 'video' ? '500MB' : '200MB'
    }
  })
  ...
  try {
    ...
    for (const bucketName of POSSIBLE_BUCKETS) {
      ...
      if (!result.error) {
        ...
        if (result.error?.message?.includes('too large') || result.error?.message?.includes('413')) {
          break
        }
      }
    }
    ...
    if (error || !data) {
      ...
      if (error?.message?.includes('too large') || error?.message?.includes('413') || errorObj?.statusCode === '413') {
        const actualLimit = fileType === 'video' ? '500MB' : '200MB'
