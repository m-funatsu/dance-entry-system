...
export default async function SNSPage() {
  ...
  // 期限チェック
  const snsEditable = await isFormEditable('sns_deadline')
  ...
  const deadlineInfo = await getDeadlineInfo(settingsMap['sns_deadline'])
                ...
                <div className="mb-6">
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-red-700">
                      SNS情報の入力期限が過ぎているため、編集できません。期限: {deadlineInfo?.date}
                    </p>
                  </div>
                </div>
              )}
              <SnsForm userId={user.id} entry={entry} isEditable={snsEditable} />
            </div>
          </div>
        </div>
      </main>
