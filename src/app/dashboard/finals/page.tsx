...
export default async function FinalsPage() {
  ...
  // 期限チェック
  const finalsEditable = await isFormEditable('finals_deadline')
  ...
  const deadlineInfo = await getDeadlineInfo(settingsMap['finals_deadline'])
                ...
                <div className="mb-6">
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-red-700">
                      決勝情報の入力期限が過ぎているため、編集できません。期限: {deadlineInfo?.date}
                    </p>
                  </div>
                </div>
              )}
              <FinalsInfoForm entry={entry} isEditable={finalsEditable} />
            </div>
          </div>
        </div>
      </main>
