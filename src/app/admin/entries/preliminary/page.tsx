                  ...
                  <tr key={preliminaryInfo.id} className="hover:bg-gray-50">
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">
                        {(preliminaryInfo.entries as Record<string, unknown>)?.users?.name || '不明なユーザー'}
                      </div>
                      <div className="text-sm text-gray-500">
                        {(preliminaryInfo.entries as Record<string, unknown>)?.participant_names || 'エントリー名なし'}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <div className="space-y-1">
                        {(preliminaryInfo.entry_files as Record<string, unknown>[])?.filter(file => 
                        ...
                        {!(preliminaryInfo.entry_files as Record<string, unknown>[])?.some(file => file.purpose === 'preliminary') && (
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        (preliminaryInfo.entries as Record<string, unknown>)?.status === 'selected' ? 'bg-green-100 text-green-800' :
                        (preliminaryInfo.entries as Record<string, unknown>)?.status === 'rejected' ? 'bg-red-100 text-red-800' :
                        (preliminaryInfo.entries as Record<string, unknown>)?.status === 'submitted' ? 'bg-blue-100 text-blue-800' :
                        'bg-yellow-100 text-yellow-800'
                      }`}>
                        {(preliminaryInfo.entries as Record<string, unknown>)?.status === 'pending' && '審査待ち'}
                        {(preliminaryInfo.entries as Record<string, unknown>)?.status === 'submitted' && '提出済み'}
                        {(preliminaryInfo.entries as Record<string, unknown>)?.status === 'selected' && '選考通過'}
                        {(preliminaryInfo.entries as Record<string, unknown>)?.status === 'rejected' && '不選考'}
                      </span>
                    </td>
                  </tr>
                ))}
