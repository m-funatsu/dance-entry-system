...
export interface UnifiedFileUploadProps {
  ...
  statusBarPosition?: 'fixed' | 'inline'
  hidePreviewUntilComplete?: boolean
}

export const UnifiedFileUpload = ({
  label,
  value,
  onChange,
  onUploadComplete,
  ...
  // アップロード状況管理
  const { status, startUpload, updateProgress, completeUpload, failUpload, clearStatus, retryUpload } = useUploadStatus()
  ...
  }, [selectedFile, clearStatus, handleFile])
    ...
    <>
      <div className={`relative ${disabled ? 'opacity-50' : ''}`}>
        ...
        <div
          ...
          className={`
            ...
            ${value && showPreview && !uploading ? 'bg-green-50 border-green-300' : ''}
      ...
      {showStatusBar && (status.isUploading || status.error) && (
        <UploadStatusBar
          ...
          error={status.error}
          onRetry={retryUpload ? handleRetry : undefined}
          className={statusBarPosition === 'inline' ? 'relative top-0 right-0 mt-4' : 'upload-status-bar'}
        />
      )}
    </>
  )
